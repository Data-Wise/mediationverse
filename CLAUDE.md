...
science, mediationverse loads all core packages with a single
...
**Implemented:** - `mediationverse_attach()`: Internal function to load
core packages - `is_attached()`: Check if packages are attached -
...
Show function name conflicts - `mediationverse_logo()`: ASCII art logo

### Package Loading Behavior

When
[`library(mediationverse)`](https://github.com/data-wise/mediationverse)
is called: 1. Loads all core packages silently 2. Displays startup
message with package versions 3. If packages missing, displays
...
All package websites should have: - **Ecosystem dropdown menu**: Links
to all packages - **Status menu**: Dashboard, Roadmap, Contributing
links - **Consistent styling**: Academic blue (#0054AD),
Inter/Montserrat/Fira Code fonts - **Standard badge order**: CRAN (if
applicable), Lifecycle, R-CMD-check, Website, R-hub, Codecov
