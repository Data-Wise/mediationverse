...
**mediationverse** is a meta-package that provides a unified entry point for the mediation analysis ecosystem in R. Similar to tidyverse for data science or easystats for statistical modeling, mediationverse loads all core packages with a single `library(mediationverse)` call.
...
- **`mediationverse_conflicts()`**: Show function name conflicts between packages
...
2. **Utility Functions**
   ...
   - `package_version_string()` returns valid versions or "(not installed)"
