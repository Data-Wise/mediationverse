...
|----|----|----|
| [`mediationverse_packages()`](https://Data-Wise.github.io/mediationverse/reference/mediationverse_packages.md) | ✅ | Good; has print method |
| [`mediationverse_update()`](https://Data-Wise.github.io/mediationverse/reference/mediationverse_update.md) | ✅ ⚠ | Stale: treats medfit/RMediation as GitHub-only — both now on CRAN |
| [`mediationverse_conflicts()`](https://Data-Wise.github.io/mediationverse/reference/mediationverse_conflicts.md) | ✅ | Exists (likely thin) |
...
Also add `upgrade = c("ask", "always", "never")` forwarded to
...
Fields: package, installed version, source (CRAN/GitHub/not installed),
currency.
...
These feed into `ci()` and `medci()` as fallback defaults. Lets analysts
write:
...
Implemented with `utils::news(package = pkg)` +
...
Wraps [`utils::citation()`](https://rdrr.io/r/utils/citation.html) per
installed package. Analysts consistently miss citing all packages.
...
Returns an S7 `MediationPipeline` object with tidy/print/summary
methods.
...
Checks: R version, package compatibility, optional deps (OpenMx,
lavaan), known conflicts.
...
Internally calls medfit + RMediation. The “Hello World” of the
ecosystem.
...
Useful for teaching contexts and power users who want the full
namespace.
...
2.  **\#2
    ...
    (1 hr) — highest user value, models tidyverse
3.  **\#3 `mv_options()`** (1 hr) — unlocks global defaults for
    ci()/medci()
4.  **\#6 `mediation_pipeline()`** (future session) — flagship feature

------------------------------------------------------------------------

*Generated: 2026-06-19 \| Mode: feature/default \| mediationverse
v0.0.0.9000*
